...
void LocateQuest::create( PCharacter *pch, NPCharacter *questman ) 
{
    ...
    try {
        ...
        customerArea = customer->in_room->areaInstance->area->name;

        if (getScenario( ).needsEndPoint( )) {
            endPoint = getRandomRoomClient( pch );
            targetArea = endPoint->areaInstance->area->name;
        }
    ...
    tell_fmt( "%3$#^P1 ждет тебя в районе {W%4$s{G ({W{hh%5$s{hx{G).", 
               pch, questman, customer, customer->in_room->name, customer->in_room->areaInstance->area->name );
...
{
    if (!customerArea.empty( ) && customerArea == room->areaInstance->area->name)
        return false;
